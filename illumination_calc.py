'''
author: S. Hasler
Script to simulate and visualize the illumination of ringed planet across multiple orbital epochs.
...
# Parameters
# ---------------------
plot_closeup = True                                  # whether to plot close-up views of planet and ring at each epoch (will also save figs)
plot = False                                         # whether to plot or not        
save_files = False                                   # whether to save output files or not
save_figs = False
...
e = 0.2987                                              # eccentricity
inc = np.radians(23.758)                                  # inclination
Omega = np.radians(0)                                # longitude of ascending node
omega = np.radians(258.82)                              # argument of periastron
...
f_planet_illum = f"/Users/shasler/Documents/Projects/Roman/fraction_lit_output/for_paper/upsAnd_newTrueAnom/obl30_phi0/planet_illumination_e{e}_inc{np.degrees(inc):.1f}deg_obliq{np.degrees(obliquity):.1f}deg_{ring_outer - ring_inner:.2f}Rring_{n_epochs}epochs_{now}.txt"
f_ring_illum = f"/Users/shasler/Documents/Projects/Roman/fraction_lit_output/for_paper/upsAnd_newTrueAnom/obl30_phi0/ring_illumination_e{e}_inc{np.degrees(inc):.1f}deg_obliq{np.degrees(obliquity):.1f}deg_{ring_outer - ring_inner:.2f}Rring_{n_epochs}epochs_{now}.txt"
...
d = (13.4*u.pc).to(u.km).value                         # km - observer distance
